...
namespace Serving { namespace {
  ...
  void ThreadProcess(
      std::unique_ptr<BatchingServer::Stub> &stub, TensorMessage msg,
      TensorMessage &reply) {
    grpc::Status status;
    ConnectionReply rep;
  ...
  TEST_F(TestIntegrationMXNet, ThreadedProcessSingle) {
    ...
    std::thread processing_thread(
        ThreadProcess, std::ref(stub), msg, std::ref(tensor_reply));
  ...
  TEST_F(TestIntegrationMXNet, ThreadedProcessMultiple_SingleBatch) {
    ...
    for (int i = 0; i < batch_size; i++) {
      request_threads.emplace_back(std::thread(
          ThreadProcess, std::ref(stub), msg, std::ref(results[i])));
    }
